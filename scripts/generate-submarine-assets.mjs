// Generates the submarine (U-boat) sprite and appends it to the unit sprite
// atlas used by UnitPass (WebGL renderer).
//
// The unit atlas is a single row of 13x13 RGBA cells, one per mobile unit.
// Sprites are grayscale PNGs whose pixels use exactly three gray bands —
// 180 (light), 130 (mid) and 70 (dark) — which the GPU colorizes per player.
// This script decodes the committed atlas, appends one new column for the
// submarine, and re-encodes it. It also emits the standalone sprite used by
// the canvas2D SpriteLoader path.
//
// Usage: node scripts/generate-submarine-assets.mjs
...
const SPRITE_PATH = path.join(root, "resources/sprites/submarine.png");
...
// 11x11 submarine: elongated hull, conning tower, periscope.
...
function inflate(data) {
  // node:zlib inflate handles the PNG IDAT zlib stream.
...
const submarine = artToRgba(SUBMARINE_ART);

// 1. Standalone sprite (11x11).
writeFileSync(SPRITE_PATH, encodePng(submarine.w, submarine.h, submarine.px));
console.log(`wrote ${SPRITE_PATH} (${submarine.w}x${submarine.h})`);

// 2. Append the submarine as a new column of the committed unit atlas.
...
const offsetX = atlas.width + Math.floor((CELL - submarine.w) / 2);
const offsetY = Math.floor((CELL - submarine.h) / 2);
stampInto(newData, newWidth * 4, submarine, offsetX, offsetY);
